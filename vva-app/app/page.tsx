        ...
        <div className='flex flex-col justify-center items-center'>
            ...
            <DndProvider backend={HTML5Backend}>
                <div className=" flex flex-col  container  p-4">
                    <h1 className="text-2xl font-bold flex justify-center">Starting grid</h1>
                                ...
                                {rows.map((row, index) => (
                                    <DraggableTableRow key={row.id} id={row.id} index={index} moveRow={moveRow}>
                                        <td className="border border-gray-300 p-2">
                                            <div className="flex items-center justify-center cursor-move" title="Drag to reorder">
            ...
            <Button className='mt-10' onClick={handleSubmit}>Prédire</Button>

            {/* Affichage des positions */}
            <div>
                <ul>
                    {positions.map((pos, index) => (
                        <li key={index}>{pos.position} : {pos.surname} - {pos.constructor}</li>
                    ))}
                </ul>
            </div>
        </div>
